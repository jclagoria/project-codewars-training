...
export function createMagicPotion(potions, target) {
    ...
    for (let i = 0; i < potions.length; i++) {
        ...
        const neededPower = target - currentPower;

        if (seenPowers.has(neededPower)) {
            return [seenPowers.get(neededPower), i];
        }

        // Guardamos el índice actual para posibles futuras combinaciones
        seenPowers.set(currentPower, i);
    }
