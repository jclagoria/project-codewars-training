/**
 * En una lucha épica entre muertos vivientes 🧟 y humanos 👮‍♂️, ambos bandos tienen una
 * lista de combatientes con poderes de ataque específicos.
 *
 * La batalla se desarrolla en rondas, y cada ronda enfrenta a cada combatiente de su bando.
 *
 * El bando con mayor poder de ataque gana la ronda, y su poder se suma al siguiente combatiente
 * de su equipo.
 *
 * En caso de empate, ambos combatientes caen y no afectan a la próxima ronda.
 *
 * Dadas dos cadenas de texto zombies y humans, donde cada dígito (del 1 al 9) representa el
 * poder de ataque de un combatiente, determina quién queda al final y con cuánto poder de ataque.
 *
...
export function battleHorde(zombies, humans) {
    const zombiePower = zombies.split('').map(Number)
    const humanPower = humans.split('').map(Number)

    let zombieTotalPower = 0
    let humanTotalPower = 0

    for (let i = 0; i < zombiePower.length; i++) {
        zombieTotalPower = zombieTotalPower + zombiePower[i]
        humanTotalPower = humanTotalPower + humanPower[i]
    }

    if (zombieTotalPower === humanTotalPower) {
        return 'x'
    }

    const resultBattle =  Math.abs(zombieTotalPower - humanTotalPower)
    const howWon = zombieTotalPower > humanTotalPower ? 'z' : 'h'

    return `${resultBattle}${howWon}`
}